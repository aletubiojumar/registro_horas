...
const CalendarPage: React.FC = () => {
  ...
  // ✅ Parseo seguro (evita bugs por zona horaria con "YYYY-MM-DD")
  ...
  // ✅ Comparación por string YYYY-MM-DD
  ...
  const handleDayClick = (day: number) => {
    ...
    if (vacationMode) {
      // ✅ Modo selección de FIN (comparación por string YYYY-MM-DD)
      if (dateStr < vacationMode) {
  ...
  // ✅ Padding para alinear el día 1 (L=0 ... D=6)
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f3f4f6" }}>
      ...
      <main style={{ maxWidth: 1180, margin: "0 auto", padding: "1.5rem 0.5rem 2rem" }}>
        ...
        {vacationMode && (
          ...
          >
            <span>
              Selecciona el último día de vacaciones (inicio: {vacationMode}) → click en el calendario
            </span>
            <button
        ...
        >
          ...
          >
            {/* ✅ Espacios vacíos al inicio (alineación correcta del mes) */}
            ...
            {monthDays.map((day) => {
              const dateStr = `${currentYear}-${String(currentMonth + 1).padStart(2, "0")}-${String(day).padStart(
                2,
                "0"
              )}`;
              ...
              if (vacApproved) {
                ...
                bg = "#fef3c7"; // AMARILLO fines de semana (S y D)
              } else {
                const types = dayEvents.map((e) => e.type) as EventType[];
                if (types.length) {
                  bg = colorByType[types[0]];
                } else if (future) {
              ...
              return (
                ...
                >
                  <div style={{ fontSize: "0.75rem", fontWeight: "bold", marginBottom: "0.25rem" }}>{day}</div>
          ...
          >
            ...
            <div style={{ marginBottom: "1rem" }}>
              <label>
                ...
                >
                  {(["visita", "juicio", "vacaciones", "cita médica", "citación judicial", "otros"] as EventType[]).map(
                    (t) => (
                  ...
                  {users.map((u) => (
                    <label key={u.id} style={{ fontSize: "0.8rem" }}>
                      <input
                        ...
                        onChange={(e) =>
                          setViewers((prev) => (e.target.checked ? [...prev, u.id] : prev.filter((id) => id !== u.id)))
                        }
                      />{" "}
                      {u.fullName}
                    </label>
